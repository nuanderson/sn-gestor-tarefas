...
class RelatorioTarefasPDFView(APIView):
    ...
    def get(self, request):
        ...
        qs = Tarefa.objects.select_related('empresa', 'responsavel').prefetch_related('sessoes').order_by('prazo', 'empresa__nome')
        ...
        # Calcula tempo por tarefa e total geral
        tarefas_list = list(qs)
        tempo_total_geral = 0
        tarefas_com_tempo = []
        for t in tarefas_list:
            mins = t.sessoes.filter(status='finalizada').aggregate(
                total=Sum('duracao_minutos')
            )['total'] or 0
            tempo_total_geral += mins
            tarefas_com_tempo.append({
                'tarefa': t,
                'tempo_minutos': mins,
                'tempo_formatado': f"{mins // 60}h {mins % 60:02d}min" if mins else "—",
            })

        total_h = tempo_total_geral // 60
        total_m = tempo_total_geral % 60

        context = {
            'tarefas': tarefas_com_tempo,
            'total': len(tarefas_com_tempo),
            'concluidas': sum(1 for item in tarefas_com_tempo if item['tarefa'].status == 'done'),
            'atrasadas': sum(1 for item in tarefas_com_tempo if item['tarefa'].status == 'late') +
                         sum(1 for item in tarefas_com_tempo if item['tarefa'].status in ('pending', 'progress') and item['tarefa'].prazo and item['tarefa'].prazo < hoje),
            'tempo_total_minutos': tempo_total_geral,
            'tempo_total_formatado': f"{total_h}h {total_m:02d}min" if tempo_total_geral else "—",
    ...
    def get(self, request):
        ...
        for colab in colaboradores:
            ...
            tarefas_colab = Tarefa.objects.filter(
                ...
                concluida_em__date__gte=inicio,
                concluida_em__date__lt=fim,
            ).select_related('empresa').prefetch_related('sessoes').order_by('concluida_em')

            # Calcula tempo por tarefa individualmente
            tarefas_com_tempo = []
            for t in tarefas_colab:
                t_mins = t.sessoes.filter(status='finalizada').aggregate(
                    total=Sum('duracao_minutos')
                )['total'] or 0
                tarefas_com_tempo.append({
                    'tarefa': t,
                    'tempo_minutos': t_mins,
                    'tempo_formatado': f"{t_mins // 60}h {t_mins % 60:02d}min" if t_mins else "—",
                })
            ...
            dados_colaboradores.append({
                ...
                'tempo_total_formatado': f"{h_tot}h {m_tot:02d}min" if minutos else "—",
                'tarefas': tarefas_com_tempo,
            })
